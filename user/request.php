...
<div class="icon-bar">
	  ...
	  <a class="active" href="../user/request.php"><i class="fa fa-handshake-o"></i></a>
...
session_start();
		...
		if ( isset($_SESSION["USERTYPE"]) && isset($_SESSION["USERID"]) )
		{
			...
			$jsonData = getJSONFromDB("select * from friend_table WHERE status ='NOTFRIENDS' and  TID=".$_SESSION['USERID']."");
			...
			foreach($jsn as $v)
			{	
			echo '<form method="post" action="accept.php" id = "id" ><p>';
			
			
			
			
			$jsonData1 = getJSONFromDB("select * from user_info WHERE USER_ID=".$v->FID."");
			...
			
		{
		    ...
		    if(file_exists($destination)){
				echo '<img src="'.$destination.'" alt="icon" align="left" width=40 height=40 onclick="Profile()">';
					...
					echo space(2).'<input type="submit" id = "visitbutton"  value ="Accept" name ="visitbutton" style="width:80px; height: 25px; text-align: center;
			background-color: black; font-weight: bold; color:gray; opacity: 0.8;"> ';
						
		            echo space(2).'<input type="button" id = "visitbutton"  value ="View" name ="visitbutton" style="width:80px; height: 25px; text-align: center;
			background-color: black; font-weight: bold; color:gray; opacity: 0.8;"> <br> <br>';
